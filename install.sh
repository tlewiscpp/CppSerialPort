...
if [[ -z "$linkType" ]]; then
    suCopyFile "$buildDir/lib$programName.a" "$globalLibDir"  || { echo "Could not copy file"; exit 1; }
else
    suCopyFile "$buildDir/lib$programName.so" "$globalLibDir"  || { echo "Could not copy file"; exit 1; }
fi
